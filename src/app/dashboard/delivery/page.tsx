...
export default function DeliveryDashboard() {
  ...
  const handleProfileSaveToggle = async () => {
    ...
    if (isEditingProfile) {
      ...
      try {
        ...
        const res = await updateDeliveryAgentProfile(agentId, profile.name, profile.email, profile.mobile);

        if (res.success) {

          setToast({ text: "Profile details updated successfully!", type: "success" });

        } else {

          setToast({ text: "Failed to update profile details.", type: "error" });

        }

      } catch (err) {

        setToast({ text: "Error saving profile details.", type: "error" });
  ...
  async function loadData() {
    ...
    try {
      ...
      const profileData = await getDeliveryAgentProfile(agentId);
            ...
            <div className={styles.routeCard} style={{ padding: '2.5rem', display: 'flex', flexDirection: 'column', gap: '2rem', height: 'fit-content' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button 
                  ...
                  onClick={handleProfileSaveToggle}
